function dxv= BS_Trilling( T, xv, flag, pi, parameter )
% function dxv= BS_Trilling( T, xv, flag, pi, parameter )
%
% Simulate Trilling equation for gas encapsulated in a shell
%
...
%--- ODE ---
q3 = (1+x).*(1-2*M) - 1/cn.*(1+x).*q2;
ddx= -1./q3.*(3/2*dx.^2.*(1-4/3*M) -1/cn*(1+x).*dx.*q1 +1+qi-qL );
